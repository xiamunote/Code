...
public class Refundticket extends JFrame implements ActionListener {
	private JTextField jtfDingDan = new JTextField(15), jtfID = new JTextField(
			20), jtfName = new JTextField(8),
			jtfFlightInfo = new JTextField(27),
			jtfChildNum = new JTextField(8), jtfAdultNum = new JTextField(8),
			jtfOriginCost = new JTextField(8), jtfTuiPiaoCost = new JTextField(
					8), jtfTime1 = new JTextField(8),
			jtfTime2 = new JTextField(8), jtfChildTuiPiaoShu = new JTextField(
					"0", 10), jtfAdultTuiPiaoShu = new JTextField("0", 10);
	...
	private String dingdanNum, name, idNum, flight1, flight2, ticketType,
			leaveTime1, leaveTime2, childNum, adultNum, cost;

	private File file;

	private long locationOfRecord;

	private FlightSeatInfo seatInfo = new FlightSeatInfo();

	private DataBaseManager DbBean = new DataBaseManager();
	...
	public Refundticket() {
		...
		this.addWindowListener(new WindowAdapter() {
			public void windowClosing(WindowEvent e) {
				Refundticket.this.setVisible(false);
				Refundticket.this.dispose();
			}
		});
	}
	...
	private void query() {
		...
		if (dingdan.length() == 0) {
			JOptionPane.showMessageDialog(null, "订单号不能为空", "错误信息",
		...
		if (id.length() == 0) {
			JOptionPane.showMessageDialog(null, "身份证号不能为空", "错误信息",
					JOptionPane.ERROR_MESSAGE);
			return;
		}
		...
		if (isValid == false) {
			...
			if (ticketType.equals("单程")) {
				...
				jtfFlightInfo.setText("(往返机票) " + " 去: " + flight1 + "; 返: "
						+ flight2);
			}
	...
	private void tuiPiao() {
		...
		if (isValid == false) {
			...
			if (cTuiPiaoShu == 0 && aTuiPiaoShu == 0) {
				JOptionPane.showMessageDialog(null, "请输入退票数", "错误信息",
						JOptionPane.ERROR_MESSAGE);
				return;
			}
	...
	private void operationForTuiPiao(int childTuiPiaoShu, int adultTuiPiaoShu) {
		...
		float tuiPiaoCost = caculateTuiPiaoCost(childTuiPiaoShu,
				adultTuiPiaoShu);//计算退票钱
		float newCost = Float.parseFloat(cost) - tuiPiaoCost;

		try {
			String sqls = "update ClientInfo set adultticketnumber=?" +
					",childticketnumber=?, piaojia=?,po=? where id=? and po=? ";
			java.sql.Connection con = DbBean.getDb();
			java.sql.PreparedStatement pst = con.prepareStatement(sqls);
			pst.setInt(1, newAdultNum);
			pst.setInt(2, newChildNum);
			pst.setFloat(3, newCost);
			pst.setString(4, "");
			pst.setString(5, jtfID.getText().trim());
			pst.setString(6, jtfDingDan.getText().trim());
			pst.execute();
		    con.commit();
			pst.close();
			con.close();
		   ...
		   seatInfo.tuiPiao(flight1, leaveTime1, totalTuiPiaoShu);
			...
			if (newChildNum == 0 && newAdultNum == 0)
				...
				JOptionPane.showMessageDialog(null, "恭喜你退票成功!" + "\n" + "你现在剩余"
						+ newChildNum + "张儿童票和" + newAdultNum + "张成人票" + "\n"
						+ "你将获得" + tuiPiaoCost + "的退票钱" + "\n" + "欢迎你再次选择我们!",
...
//计算退票钱
	private float caculateTuiPiaoCost(int childTuiPiaoShu, int adultTuiPiaoShu) {
		...
		try {
			String sqlString = "select childFare,adultFare from flightTick where flight='"
					+ flight1 + "'";
			...
			float adultFare2 = 0;
			if (flight2.length() != 0) {
				String sqlString2 = "select childFare,adultFare from flightTick where flight='"
						+ flight2 + "'";
			...
			if (flight2.length() == 0) {
				tuiPiaoCost = (childFare1 * childTuiPiaoShu + adultFare1
						* adultTuiPiaoShu)
						* (float) 0.7;
			} else {
				tuiPiaoCost = ((childFare1 + childFare2) * childTuiPiaoShu + (adultFare1 + adultFare2)
						* adultTuiPiaoShu)
						* (float) 0.7;
			}
		} catch (Exception e) {
	...
	private boolean getClientInfo(String dingdan, String id) {
		...
		try {
			...
			if (rs.next()) {
				...
				idNum = rs.getString("id");
				flight1 = rs.getString("flight");
				flight2 = rs.getString("returnflight");
				ticketType = rs.getString("style");
				leaveTime1 = rs.getString("starttime");
				leaveTime2 = rs.getString("returntime");
				childNum = rs.getString("childticketnumber");
				adultNum = rs.getString("adultticketnumber");
				cost = rs.getString("piaojia");
				
				boolean isValid = canReturn(leaveTime1);

				if (!isValid) {
					JOptionPane.showMessageDialog(null, "该票已经过期!不能再退!", "错误信息",
							JOptionPane.ERROR_MESSAGE);
					return false;
				}
				...
				if (Integer.parseInt(childNum) == 0
						&& Integer.parseInt(adultNum) == 0) {
					JOptionPane.showMessageDialog(null, "该订单号已经无效!", "错误信息",
							JOptionPane.ERROR_MESSAGE);
					return false;
				}
				...
				JOptionPane.showMessageDialog(null, "订单号或身份证号不正确", "错误信息",
						JOptionPane.ERROR_MESSAGE);
				return false;
			}
			rs.close();
